...
st_transform(usgsSites,st_crs(26911)) #convert to 26911
...
dbWriteData(metric="streamflow",
            value=streamflowData$Flow,
            datetime = streamflowData$Date,
            locationID = streamflowData$locationid,
            sourceName="streamflow_data.csv",units="cfs",addMetric=T)  #double check that flow is in cfs!!!!!!!!!
