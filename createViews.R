...
dbSendQuery(conn, "CREATE MATERIALIZED VIEW locationattributes AS SELECT ROW_NUMBER() OVER(), met.locationid, met.locationgeometry, met.metrics, wsh.wshedareakm, wsh.watershedgeometry FROM
                      (SELECT DISTINCT locations.locationid, locations.geometry AS locationgeometry, STRING_AGG(DISTINCT(data.metric), ',') AS metrics FROM locations LEFT JOIN data ON locations.locationid = data.locationid GROUP BY locations.locationid) met
                     LEFT JOIN 
